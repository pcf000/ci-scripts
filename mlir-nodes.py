...
servers = [{'host' : 'http://ml-ci.amd.com:21096',
            ...
            'builds' : ['MLIR/check-mlir-nightly-all', 'MLIR/mlir-weekly']},
           {'host' : 'http://ml-ci-internal.amd.com:8080',
            'passwordfile' : '.jenkins.ml-ci-internal',
            'builds' : ['mlir/mlir-nightly-all', 'mlir/mlir-weekly']}
]
...
def xdlop_nodes(server):
    nodes = []
    for n in server.get_nodes(depth=1):
        #  not n['offline'] and
        if n['name'] != 'Built-In Node':
            name = n['name']
            info = server.get_node_info(name, depth=1)
            labels = [x['name'] for x in info['assignedLabels'] if x['name'] != name]
            #  and ('gfx908' in labels or 'gfx90a' in labels):
            if 'mlir' in labels:
                note = ''
                if info['offline']:
                    note = ' (offline)'
                    ...
                    note = ' (idle)'
                nodes.append([name+note, ','.join(labels)])
#                print(f"{name}{note}: \t{labels}")
    return nodes
            ...
            if 'all' in labels or all([l in node_labels for l in labels]):
                ...
                if info['offline']:
                    ...
                    if info['offlineCauseReason'] == 'Offline because computer was idle; it will be relaunched when needed.':
                        ...
                        second = info['offlineCauseReason']
                elif info['idle']:
                    note = ' (idle)'
                else:
                    ex = info['executors'][0]
